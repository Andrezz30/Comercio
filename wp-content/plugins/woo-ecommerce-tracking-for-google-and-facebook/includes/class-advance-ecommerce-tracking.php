...
class Advance_Ecommerce_Tracking {
    ...
    private function load_dependencies() {
         ...
         *
         */
        require_once plugin_dir_path( dirname( __FILE__ ) ) . 'includes/class-advance-ecommerce-tracking-user-feedback.php';
    ...
    private function define_public_hooks() {
        if ( !is_admin() ) {
            ...
            $aft_enable = 'off';
            $fb_ecommerce_tracking = 'off';
            $aet_gc_tracking_settings = json_decode( get_option( 'aet_gc_tracking_settings' ), true );
            if ( !empty( $aet_gc_tracking_settings ) ) {
                $agc_enable = $aet_gc_tracking_settings['gc_enable'];
            } else {
                $agc_enable = 'off';
            }
